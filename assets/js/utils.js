/* utils.js - repost-safe version (avoid "\(" and "\[") */
(() => {
"use strict";
...
*
* Repost-safe note:
* - Link parsing regex avoids "\(" and "\[" patterns.
*/
...
// NOTE: No backslashes used to match [, ], (, ) to avoid "math-mode" converters.
// Pattern explanation:
// [[] matches literal '['
// []] matches literal ']'
// [(] matches literal '('
// [)] matches literal ')'
const linkRe = /[[]([^]]+)[]][(]([^)]*)[)]/g;
s = s.replace(linkRe, (_m, txt, url) => {
const u = String(url).trim();
if (!isValidHttpUrl(u)) {
return `${escapeHtml(txt)} (${escapeHtml(u)})`;
}
return `<a href="${escapeHtml(u)}" target="_blank" rel="noopener noreferrer">${escapeHtml(
txt
)}</a>`;
});
